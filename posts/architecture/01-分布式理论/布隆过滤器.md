...
**更直观的解释**：
1. **单个位置被设置为1的概率**：当添加n个元素，每个元素使用k个哈希函数时，位数组中某个特定位置被至少一个哈希函数映射到的概率为  1 - e^{-\frac{kn}{m}} 
2. **误判率**：对于一个不在集合中的元素，它的k个哈希值都恰好映射到已被设置为1的位置的概率，就是上述概率的k次方

**简化理解**：
- 位数组越长（m越大），误判率越低
- 哈希函数越多（k越大），误判率先降低后升高（存在最优值）
- 元素数量越多（n越大），误判率越高

#### 误判的本质：哈希冲突

**布隆过滤器的误判本质上是由哈希冲突导致的**。当两个不同的元素经过k个哈希函数计算后，恰好映射到位数组的相同位置组合，且这些位置都被设置为1时，就会发生误判。

**简单例子说明**：
假设我们有10个元素（n=10），使用1个哈希函数（k=1），位数组长度为100（m=100）。

1. **单个元素的哈希冲突概率**：
   - 第一个元素映射到任意位置的概率是1
   - 第二个元素映射到与第一个元素相同位置的概率是1/100
   - 第三个元素映射到与前两个元素之一相同位置的概率约是2/100
   - ...
   - 第十个元素映射到与前九个元素之一相同位置的概率约是9/100

2. **总的哈希冲突概率**：
   - 至少发生一次哈希冲突的概率为：
     $$ 1 - \frac{100}{100} \times \frac{99}{100} \times \frac{98}{100} \times ... \times \frac{91}{100} \approx 0.407 $$

3. **误判概率**：
   - 对于一个不在集合中的新元素，它映射到已被占用位置的概率约为：
     $$ \frac{10}{100} = 0.1 $$
   - 这就是误判率，约为10%
