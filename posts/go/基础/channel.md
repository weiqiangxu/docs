...
4. **channel的特性总结**：
   - 一般分配在堆上，Go的运行时决定。特定情况下会分配到栈上（生命周期短、仅在函数内部使用且通过逃逸分析）
   - 缓冲区就是数组
   - 两个队列（发送、接收）围绕着缓冲数组下标循环往复，发送和接收队列是有序的
   - 关闭状态接收返回零值
   - 缓冲区buffer之中有sendx发送队列偏移和recvx接收偏移，围绕着环形缓冲区循环移动
...
更详细的发送过程步骤：
1. 有没有goroutine在接收数据
2. 如果缓冲区已经满了，继续发送数据到channel的话会进入发送等待队列sendq
3. 发送等待队列sendq底层是sudog数据结构链表
4. sudog数据结构链表记录哪个协程在等待、等待哪个channel
5. 当有goroutine接收数据后，缓冲区就空余出位置
6. 此时会唤醒发送队列中的goroutine，继续发送数据

> 缓冲区有空余或者有goroutine在接收channel数据的时候才不会发生阻塞
...
4. **无缓冲channel的传递机制**：
   - 无缓冲通道没有缓冲数组，发送和接收操作是直接同步进行的
   - 数据直接从发送协程的栈内存复制到接收协程的栈内存
   - 这种直接传递虽然仍有数据拷贝，但避免了额外的内存分配和缓冲处理开销
...
3. **随机选择**：如果有多个case可以执行，select会随机选择一个执行，而不是按照代码顺序
4. **阻塞等待**：如果没有可以立即执行的case且没有default分支，select会阻塞当前goroutine，等待任意一个case就绪

多路select处理逻辑总结：
- 多个通道的select被称为多路select
- 有default分支就是非阻塞式select
- 没有default分支就是阻塞式select
- 当多个case同时就绪时，select会乱序轮询，随机选择一个就绪通道执行
...
### 7.1 参考资源
- [幼麟实验室Golang合集](https://www.bilibili.com/video/BV1hv411x7we) - 提供了更多关于Go语言channel的深入讲解和实践案例
